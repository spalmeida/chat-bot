...
function monta_json($step, $msg, $perguntas, $json_gerado = '')
{
    ...
    $step_atual = $step - 1;
    $texto_pergunta = strip_tags($perguntas[$step - 1]['texto']);
    $mensagem = $msg;
    $tema = $perguntas[$step - 1]['tema'];
    if (empty($perguntas[$step - 1]['alternativas'])) {
        $peso = '';
    } else {
        $peso = $perguntas[$step_atual]['alternativas'][$msg[0]]['peso'];
    }
...
function resposta($pergunta, $step)
{
    $retorno = '';
    if (monta_chat($pergunta, $step)) {
        echo json_encode(array(
            'next_step' => $step + 1,
            'previus_step' => ($step < 0) ? 0 : $step,
            'count_steps' => count($pergunta),
            'json_gerado' => monta_json($step, $msg, $perguntas['perguntas'], $json_gerado),
            'data' => monta_chat($pergunta, $step),
            'error' => FALSE
...
if ($msg != '') {
    ...
    echo resposta($perguntas['perguntas'], $step);
}